...
export function lunarFingerprint(
  ...
  const resilience = finalState.colony.morale > 0.6 ? 'robust' : finalState.colony.morale > 0.35 ? 'stable' : 'fragile';
  const sustainment = finalState.colony.foodMonthsReserve > 12 ? 'self-sustaining' : finalState.colony.foodMonthsReserve > 6 ? 'resupply-dependent' : 'critical';
